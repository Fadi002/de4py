    ...
    def _collect(self, stmts: list):
        for node in stmts:
            ...
            if isinstance(val, ast.Constant):
                # Scalars only: inlining a mutable container as a fresh literal
                # at every load site destroys shared-reference semantics and the
                # deobfuscated sample diverges from the original.
        ...
        for stmt in stmts:
            for n in ast.walk(stmt):
                if isinstance(n, ast.Name) and isinstance(n.ctx, ast.Load):
                    used.add(n.id)
                # `x += 1` reads x (target is Store, missed above) and
                # `del x` requires x to be bound - both are uses.
