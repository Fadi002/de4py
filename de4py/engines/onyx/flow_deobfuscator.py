...
class FlowDeobfuscator:
                        ...
                        if isinstance(node, ast.Assign):
                            for t in node.targets:
                                if isinstance(t, ast.Name):
                                    ...
                                    if assign_counts[t.id] == 1 and isinstance(node.value, ast.Constant):
                                        values[t.id] = node.value.value
                        elif isinstance(node, (ast.AugAssign, ast.NamedExpr)):
                            t = getattr(node, 'target', node)
                            if isinstance(t, ast.Name):
                                assign_counts[t.id] = assign_counts.get(t.id, 0) + 99
                aliases = {k: v for k, v in values.items() if assign_counts.get(k, 0) == 1}
                if not aliases:
                    return stmts
                ...
                class Sub(ast.NodeTransformer):
                    def visit_Name(self, n):
                        if isinstance(n.ctx, ast.Load) and n.id in aliases:
                            return ast.copy_location(ast.Constant(value=aliases[n.id]), n)
                        return n
    ...
    def _fold_redundant_var_chains(self, tree: ast.Module) -> ast.Module:
        ...
        class ChainFolder(ast.NodeTransformer):
            ...
            # Not on If/While/For bodies: `_fold_block` only sees reads within
            # the block it's given, but a binding from inside one of those
            # (they don't introduce a new scope) may legally be read after
            # the block ends.
    ...
    def _fold_known_var_joins(self, tree: ast.Module) -> ast.Module:
        ...
        def _invalidates(stmt, name) -> bool:
        ...
        class VarJoinFolder(ast.NodeTransformer):
            ...
            def _fold_block(self, stmts):
                ...
                new_stmts = []
                for stmt in stmts:
                    for name in list(known_lists):
                        if _invalidates(stmt, name):
                            del known_lists[name]
