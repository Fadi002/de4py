...
# Builtins that iterate their iterable argument exhaustively. A lazy
# ``range``/``map``/``zip``/``filter``/``enumerate`` passed to one of these is
# the allocation/CPU bomb in the audit: the result cap only runs *after* the
# real builtin finishes, so the iterable must be materialized first, under the
# step budget and MAX_ITERATIONS, to make the work metered.
_EAGER_CONSUMERS = frozenset({
    sum, max, min, list, tuple, set, frozenset, dict, bytearray, bytes,
    sorted, all, any,
})
...
def _check_size(value):
    if isinstance(value, int) and value.bit_length() > _MAX_INT_BITS:
        raise SafeEvalError("integer result too large")
...
def _bounded_decompress(factory, data):
    """Decompress with a hard output cap instead of a post-hoc size check.

    ``zlib``/``bz2``/``lzma`` decompressors report leftover unconsumed input
    once ``max_length`` is reached, so a bomb is caught while the allocation
    is still bounded - not after it already ran to gigabytes.
    """
    if isinstance(data, bytearray):
        data = bytes(data)
    if not isinstance(data, bytes):
        raise SafeEvalError("decompress input must be bytes")
    try:
        dec = factory()
    except Exception:
        raise SafeEvalError("decompress failed")
    out = dec.decompress(data, MAX_RESULT_BYTES + 1)
    if not isinstance(out, bytes) or len(out) > MAX_RESULT_BYTES:
        raise SafeEvalError("decompressed data too large")
    if getattr(dec, "unconsumed_tail", None) or not getattr(dec, "eof", True):
        raise SafeEvalError("decompressed data too large")
    return out


_DECOMPRESS_FACTORIES = {
    zlib.decompress: zlib.decompressobj,
    bz2.decompress: bz2.BZ2Decompressor,
    lzma.decompress: lzma.LZMADecompressor,
}

_CODEC_DECOMPRESSORS = {
...
def _safe_decompress(fn):
    factory = _DECOMPRESS_FACTORIES.get(fn)
    if factory is None:
        return lambda data, *args: _check_size(fn(data, *args))

    def wrapper(data, *args):
        return _bounded_decompress(lambda: factory(*args), data)

    return wrapper


def _safe_codecs_decode(data, encoding="utf-8", errors="strict"):
    """codecs.decode without the decompression-bomb bypass.

    ``codecs.decode(blob, 'bz2')`` reaches real zlib/bz2/lzma with no cap;
    route those codecs through the bounded decompressor and size-check every
    other decode result like any other expression.
    """
    factory = _CODEC_DECOMPRESSORS.get(encoding.lower()) if isinstance(encoding, str) else None
    if factory is not None:
        return _bounded_decompress(factory, data)
    if isinstance(data, (bytes, bytearray)) and len(data) > MAX_RESULT_BYTES:
        raise SafeEvalError("codec input too large")
    return _check_size(codecs.decode(data, encoding, errors))


# Allocation guards: every primitive below can request an allocation sized by
# attacker-controlled arguments before any post-hoc size check could run.
# They are capped up front.
_MAX_INT_BITS = 1_000_000


def _check_shift(a, b):
    if isinstance(b, int):
        if abs(b) > _MAX_INT_BITS:
            raise SafeEvalError("shift too large")
        if isinstance(a, int) and a.bit_length() + abs(b) > _MAX_INT_BITS:
            raise SafeEvalError("shift too large")
...
def _guard_rshift(a, b):
    _check_shift(a, b)
    return a >> b


def _guard_bytes(count, *args):
    if isinstance(count, int) and count > MAX_RESULT_BYTES:
        raise SafeEvalError("allocation too large")
    return bytes(count, *args)


_guard_bytes._eager_consumer = True
_guard_bytes._type_alias = "bytes"


def _guard_bytearray(data=b"", *args, **kwargs):
    if isinstance(data, int) and data > MAX_RESULT_BYTES:
        raise SafeEvalError("allocation too large")
    return bytearray(data, *args, **kwargs)


_guard_bytearray._eager_consumer = True
_guard_bytearray._type_alias = "bytearray"


def _safe_struct_pack(fmt, *args):
    if isinstance(fmt, str):
        try:
            size = struct.calcsize(fmt)
        except Exception:
            raise SafeEvalError("bad struct format") from None
        if size > MAX_RESULT_BYTES:
            raise SafeEvalError("pack too large")
    return struct.pack(fmt, *args)


_STR_WIDTH_METHODS = frozenset({"zfill", "center", "ljust", "rjust", "expandtabs"})


def _guard_width(method):
    def invoke(width, *args):
        if isinstance(width, int) and width > MAX_RESULT_BYTES:
            raise SafeEvalError("width too large")
        return method(width, *args)

    invoke._sandbox_guard = True
    return invoke
...
def _guard_format(value, format_spec=""):
    if isinstance(format_spec, str):
        for m in _DIGIT_RUN_RE.finditer(format_spec):
            if int(m.group()) > MAX_RESULT_BYTES:
                raise SafeEvalError("format width too large")
    return format(value, format_spec)


PURE_BUILTINS = {
    "abs": abs, "all": all, "any": any, "ascii": ascii, "bin": bin,
    "bool": bool, "bytearray": _guard_bytearray, "bytes": _guard_bytes, "chr": chr,
    "complex": complex, "dict": dict, "divmod": divmod, "enumerate": enumerate,
    "filter": filter, "float": float, "format": _guard_format, "frozenset": frozenset,
...
_MODULE_FUNCS = {
    ...
    ("codecs", "decode"): _safe_codecs_decode,
...
_DUNDER_ARITH = {
    ...
    "__lshift__": _guard_lshift,
    "__rshift__": _guard_rshift,
...
_BINOPS = {
    ...
    ast.Pow: _guard_pow,
    ast.LShift: _guard_lshift,
    ast.RShift: _guard_rshift,
...
class SafeEvaluator:
    ...
    def _format_value(self, node, env):
        ...
        return _guard_format(value, spec)
    ...
    def _get_attr(self, obj, attr: str):
        ...
        if attr == _TYPE_NAME_ATTR:
            if isinstance(obj, type):
                return obj.__name__
            alias = getattr(obj, "_type_alias", None)
            if alias:
                return alias
        if attr == _CLASS_ATTR and isinstance(obj, _CLASS_ACCESSIBLE):
            # resolve to the SAME guarded constructor the table exposes, so
            # the class-route cannot bypass the allocation caps
            return PURE_BUILTINS.get(type(obj).__name__, type(obj))
        ...
        if isinstance(obj, str) and attr in _STR_WIDTH_METHODS:
            return _guard_width(method)
        return method
    ...
    def _n_Call(self, node, env):
        ...
        if (isinstance(node.func, ast.Name) and node.func.id == "getattr"
            ...
            # The inner expression is evaluated against THIS evaluator's
            # remaining budget: routing through safe_eval() handed every
            # nesting level a fresh step allowance.
            try:
                name = self._eval(node.args[1], env)
            except SafeEvalError:
                raise
            except Exception:
                raise SafeEvalError("builtin name unresolved")
            if isinstance(name, str) and name in PURE_BUILTINS:
                return PURE_BUILTINS[name]
            if name == "__import__":
                return _IMPORT_BUILTIN
            if name == "getattr":
                return _GETATTR_BUILTIN
            raise SafeEvalError(f"builtin not allowed: {name!r}")
    ...
    def apply(self, func, args, kwargs=None):
        ...
        if callable(func) and self._is_trusted_callable(func):
            ...
            if func in _EAGER_CONSUMERS or getattr(func, "_eager_consumer", False):
                args = [self._materialize(a, include_range=True) for a in args]
                kwargs = {k: self._materialize(v, include_range=True) for k, v in kwargs.items()}
    ...
    def _is_trusted_callable(self, func) -> bool:
        if func in _TRUSTED_CALLABLES or getattr(func, "_sandbox_guard", False):
            return True
    ...
    def _materialize(self, value, include_range=False):
        """Force lazy builtins (map/filter/zip/range) so budgets actually bind."""
        if include_range and isinstance(value, range):
            value = iter(value)
            lazy = True
        else:
            lazy = isinstance(value, (map, filter, zip, enumerate, reversed))
        if lazy:
            out = []
            for item in value:
                self.budget.step()
