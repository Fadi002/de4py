...
import weakref
from PySide6.QtCore import QObject, QPropertyAnimation, QEasingCurve, QByteArray
...
class AnimationRegistry:
    ...
    def get_or_create(self, target: QObject, prop_name: bytes) -> QPropertyAnimation:
        ...
        # When target is destroyed, we should drop the key.
        target.destroyed.connect(lambda obj=None, k=key: self._animations.pop(k, None))

        return anim

    def cancel_all(self, target: QObject):
        """Cancels all animations for a specific target."""
        target_id = id(target)
        keys_to_remove = []
        for key, anim in self._animations.items():
            if key[0] == target_id:
                if anim.state() == QPropertyAnimation.State.Running:
                    anim.stop()
