...
class Sidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.line_shadow.setColor(QColor(88, 166, 255, 120))
        ...
        self.title_shadow.setColor(QColor(88, 166, 255, 200))

        self._opacity_spring = spring_gentle(0.95)
        self._glow_spring = spring_gentle(1.0)
    ...
    def on_sidebar_opening(self):
        self._opacity_spring.value = 0.7
        self._opacity_spring.target = 0.95
        self._glow_spring.value = 1.3
        self._glow_spring.target = 1.0

    def on_sidebar_closing(self):
        self._opacity_spring.value = 0.95
        self._opacity_spring.target = 0.7
        self._glow_spring.target = 0.8
    ...
    def _setup_ui(self):
        ...
        for nav_id, label_key, icon_file in NAV_ITEMS:
            ...
            icon_path = os.path.join(base_path, "..", "resources", icon_file)
    ...
    def set_active(self, nav_id: str):
        ...
        for button_id, btn in self._buttons.items():
            btn.setProperty("active", button_id == nav_id)
            btn.style().unpolish(btn)
            btn.style().polish(btn)

            if button_id == nav_id:
                btn._material.drive('active', 1.0)
                try:
                    from de4py.ui.motion.proximity import SurfaceLinker
                    SurfaceLinker.instance().broadcast(btn, 0.6)
                except Exception:
                    pass
            elif button_id == old_active:
                btn._material.drive('active', 0.0)
