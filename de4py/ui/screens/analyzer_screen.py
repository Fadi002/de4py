...
from de4py.lang.keys import (
    SCREEN_TITLE_ANALYZER, ANALYZER_SELECT_FILE, MSG_NO_FILE_SELECTED,
    MSG_OPERATION_FAILED, ANALYZER_OPTIONS_TITLE,
    ...
    ANALYZER_CMD_HASHES, DLG_SELECT_FILE
)
...
class AnalyzerScreen(QWidget):
    ...
    def _run_command(self, command: str):
        ...
        if command in ["detect_packer", "unpack_exe"] and not self._file_path.lower().endswith(".exe"):
    ...
    def _on_command_finished(self, raw_result: str):
        self.window().hide_loading()
        self._set_commands_enabled(True)

        formatted_result = raw_result
        if raw_result.startswith("{") or raw_result.startswith("["):
            try:
        ...
        self.output.set_text(formatted_result)
    ...
    def _add_command_buttons(self, layout):
        ...
        for label, cmd in commands:
            ...
            self._command_buttons.append(btn)
