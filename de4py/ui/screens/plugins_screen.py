...
class PluginsScreen(QWidget):
    ...
    def _create_plugin_card(self, plugin, plugin_type):
        ...
        name_label = QLabel(f'<a href="{escape(plugin.link or "")}" style="color: #0287CF;">{escape(plugin.name)}</a>')
        name_label.setOpenExternalLinks(True)
        name_label.setObjectName("SubtitleLabel")
        layout.addWidget(name_label)

        creator_label = QLabel(f"{tr(PLUGINS_MADE_BY)} {escape(plugin.creator or '')}")
    ...
    def _apply_theme(self, plugin):
        ...
        try:
            if hasattr(plugin, 'qss'):
                ...
                settings.set_and_emit("active_theme", plugin.name)
            ...
            settings.set_and_emit("active_theme", None)
