    ...
    def _on_changelog_loaded(self, data: str):
        try:
            ...
            for version in changelog_data:
                parts.append(f"<h3>Version {escape(str(version.get('version', '')))}</h3><ul>")
                for change in version.get('changes', []):
                    parts.append(f"<li>{escape(str(change))}</li>")
                parts.append("</ul>")
            self.changelog_content.setText("".join(parts))
        except Exception:
            self.changelog_content.setText(escape(data))

    def _on_changelog_error(self, error: str):
        self.changelog_content.setText(escape(error))
