...
from de4py.lang.keys import (
    ...
    DEOBF_AI_CONSENT_LOCAL, DLG_SELECT_FILE
)
...
class DeobfuscatorScreen(QWidget):
    ...
    def _create_left_frame(self):
        ...
        self.engine_mode_combo.addItems(["normal", "safe", "aggressive"])
        self.engine_mode_combo.setCurrentText("normal")
        self.engine_mode_combo.setToolTip(
            "Engine analysis mode: safe (conservative), normal, "
            "aggressive (try speculative transforms)")
        layout.addWidget(self.engine_mode_combo)
    ...
    def _on_deobfuscate(self):
        ...
        with sentry.transaction("Deobfuscation Run", "tool.deobfuscate"):
            ...
            self.deobf_btn.setEnabled(False)
            ...
            self._worker = DeobfuscatorWorker(
                ...
                engine_options={"mode": self.engine_mode_combo.currentText()}
            )
    ...
    def _on_deobf_finished(self, result: str):
        ...
        self.deobf_btn.setEnabled(True)
    ...
    def _show_consent_overlay(self, provider: str, is_local: bool):
        ...
        if (overlay.isVisible() and not overlay.is_dismissing()
