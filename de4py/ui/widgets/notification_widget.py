...
class NotificationManager(QWidget):
    ...
    def show_notification(self, notif_type: str, message: str):
        ...
        dismiss_timer = QTimer(notif)
        dismiss_timer.setSingleShot(True)
        dismiss_timer.timeout.connect(lambda n=notif: self._slide_out(n))
        dismiss_timer.start(NOTIF_DURATION)
    ...
    def _slide_out(self, notif: NotificationWidget):
        ...
        cleanup_timer = QTimer(notif)
        cleanup_timer.setSingleShot(True)
        cleanup_timer.timeout.connect(lambda: self._cleanup_notif(notif))
        cleanup_timer.start(500)
