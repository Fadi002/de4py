...
from de4py.lang.keys import (
    TITLE_UPDATE_AVAILABLE, BTN_LATER, BTN_UPDATE_NOW,
    BTN_INSTALL_RESTART, BTN_RETRY, MSG_DOWNLOADING,
    MSG_DOWNLOADING_PCT, MSG_DOWNLOAD_COMPLETE, MSG_UPDATE_ERROR,
    MSG_NO_CHANGELOG,
)
...
class UpdateDialog(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.title = QLabel(tr(TITLE_UPDATE_AVAILABLE))
        ...
        self.btn_dismiss = QPushButton(tr(BTN_LATER))
        self.btn_dismiss.clicked.connect(self._on_dismiss)
        btn_layout.addWidget(self.btn_dismiss)

        self.btn_primary = QPushButton(tr(BTN_UPDATE_NOW))
    ...
    def _show_available(self):
        ...
        self.version_label.setText(
            f"{escape(self.result.local)}  →  <b>{escape(self.result.remote)}</b>"
        )
        html_parts = []
        for entry in self.result.changelog:
            html_parts.append(f"<h3>v{escape(str(entry.get('version', '')))}</h3><ul>")
            for ch in entry.get('changes', []):
                html_parts.append(f"<li>{escape(str(ch))}</li>")
            html_parts.append("</ul>")
        self.changelog_view.setHtml("".join(html_parts) or f"<p>{tr(MSG_NO_CHANGELOG)}</p>")
        self.btn_primary.setText(tr(BTN_UPDATE_NOW))

    def show_downloading(self):
        self._state = _State.DOWNLOADING
        self.progress_bar.show()
        self.progress_bar.setRange(0, 0)
        self.status_label.setText(tr(MSG_DOWNLOADING))
    ...
    def set_download_progress(self, pct: int):
        ...
        self.status_label.setText(tr(MSG_DOWNLOADING_PCT, pct=pct))
    ...
    def show_ready(self):
        ...
        self.btn_primary.setText(tr(BTN_INSTALL_RESTART))
        self.btn_primary.setEnabled(True)
        self.btn_dismiss.setEnabled(True)

    def show_error(self, msg: str):
        self._state = _State.FAILED
        self.status_label.setText(tr(MSG_UPDATE_ERROR, error=msg))
        self.btn_primary.setText(tr(BTN_RETRY))
        self.btn_primary.setEnabled(True)
        self.btn_dismiss.setEnabled(True)

    def _on_primary(self):
        if self._state in (_State.AVAILABLE, _State.FAILED):
