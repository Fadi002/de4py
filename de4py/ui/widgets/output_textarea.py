...
from de4py.lang.keys import MSG_COPIED, BTN_COPY_OUTPUT, LBL_OUTPUT, OUTPUT_SEARCH_PLACEHOLDER
...
class PythonHighlighter(QSyntaxHighlighter):
    ...
    _TRIPLE_DOUBLE = QRegularExpression('"""')
    _TRIPLE_SINGLE = QRegularExpression("'''")
...
class SearchBar(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.input.setPlaceholderText(tr(OUTPUT_SEARCH_PLACEHOLDER))
    ...
    def set_count(self, count: int, total: int, truncated: bool = False):
        error = total == 0 and count == 0 and bool(self.input.text())
        shown_total = f"{total}+" if truncated else str(total)
        label = "0 / 0" if error else f"{count} / {shown_total}" if total > 0 else ""
...
class OutputTextArea(QWidget):
    ...
    def _setup_search(self):
        ...
        self._search_timer.timeout.connect(
            lambda: self._run_search(self.search_bar.input.text()))
    ...
    def _run_search(self, text):
        ...
        while not cursor.isNull():
            if len(self._search_matches) >= self.MAX_SEARCH_MATCHES:
    ...
    def _scroll_to_match(self):
        ...
        self.search_bar.set_count(self._search_index + 1,
                                  len(self._search_matches),
                                  truncated=self._search_truncated)
    ...
    def _highlight_matches(self):
        ...
        for start, end in self._search_matches[:self.MAX_HIGHLIGHTED]:
    ...
    def set_streaming_limit(self, max_blocks: int):
        self.text_edit.document().setMaximumBlockCount(max_blocks)
