...
def _resolve_injector_paths(pid, bp):
    try:
        pid = int(pid)
    ...
    return pid, injector, pyshell_dll
...
def inject_shell(pid, bp=None):
    pid, injector, pyshell_dll = _resolve_injector_paths(pid, bp)
    if pid is None:
        return (None, False)
...
def show_console(pid, bp=None):
    pid, injector, _ = _resolve_injector_paths(pid, bp)
    if pid is None:
        return False
