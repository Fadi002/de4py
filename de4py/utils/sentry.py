...
def _scrub_paths(value):
    """Reduce absolute filesystem paths to their basename so directory
    structures never leave the machine via breadcrumbs or extras."""
    if isinstance(value, str):
        candidate = value.strip()
        if _ABS_PATH.match(candidate):
            return re.split(r"[\\/]", candidate)[-1] or candidate
        return value
...
def set_extra_context(key, value):
    if _is_active():
        import sentry_sdk
        sentry_sdk.set_context(key, _scrub_paths(value))
