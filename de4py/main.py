...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tui", action="store_true", help="Start in terminal (TUI) mode")
...
def run_tui_if_requested(args) -> bool:
    if not (args.tui or args.cli):
        return False
    import logging
    from de4py.utils import sentry
    if args.cli:
        print("Warning: --cli is deprecated, use --tui instead", file=sys.stderr)
    with sentry.transaction("TUI Session", "app.tui"):
        logging.info("Starting in TUI mode...")
    ...
    with sentry.transaction("GUI Session", "app.gui"):
        ...
        def _initialize_optional_services():
            ...
            try:
                ...
                if app._dev_manager:
                    app.aboutToQuit.connect(app._dev_manager.close)
                    print("\n" + "="*40)
            ...
            if str(os.getenv("DEV_MODE", "")).lower() in ("1", "true", "on", "yes"):
                try:
                    from de4py.ui.devtools.proxy.api import DevApiProxy
                    for mod_name in ("de4py.api.client", "de4py.api.pylingual",
                                     "de4py.api.telemetry", "de4py.api"):
                        try:
                            mod = __import__(mod_name, fromlist=[mod_name.split(".")[-1]])
                        except ImportError:
                            continue
                        mod.De4pyApiClient = DevApiProxy
